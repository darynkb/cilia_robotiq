...
# --- MODIFIED: Use separate arrays for each Arduino ---
force_data0 = np.zeros((4, 3))  # For Arduino 0
force_data1 = np.zeros((4, 3))  # For Arduino 1
...
class SerialReaderThread(threading.Thread):
    def __init__(self, port='/dev/ttyACM0', baud=115200, idx=0):
        super().__init__()
        self.port = port
        self.baud = baud
        self.idx = idx  # 0 for Arduino 0, 1 for Arduino 1
        self.running = True
        self.ser = None

    def run(self):
        global force_data0, force_data1
        last_time = time.time()
        count = 0
        try:
            self.ser = serial.Serial(self.port, self.baud, timeout=1)
            time.sleep(2)
            print(f"start reading... {self.port}\n")
            while self.running:
...
# --- MODIFIED: Return only the requested sensor's data ---
def read_sensor_data(sensor_id=0):
    global force_data0, force_data1
    if sensor_id == 0:
        return force_data0
    else:
...
class TaxelVisualizer(QMainWindow):
    ...
    def update_arrows(self):
        # --- GUI update frequency monitoring ---
        if not hasattr(self, '_gui_last_time'):
            self._gui_last_time = time.time()
            self._gui_count = 0
        self._gui_count += 1
        now = time.time()
        if now - self._gui_last_time >= 1.0:
            print(f"[GUI] update_arrows frequency: {self._gui_count} Hz")
            self._gui_count = 0
            self._gui_last_time = now

        # --- MODIFIED: Read both sets of data ---
        forces0 = read_sensor_data(0) 
        # forces1 = read_sensor_data(1) 
        # TODO: Adapt visualization logic for 8 taxels if needed
        # For now, use only the first Arduino's data for compatibility
        forces = forces0
        k_deform = 0.03
...
if __name__ == '__main__':
    # --- MODIFIED: Start two threads for two Arduinos ---
    serial_thread0 = SerialReaderThread(port='/dev/ttyACM0', baud=115200, idx=0)
    serial_thread0.daemon = True
    serial_thread0.start()
    serial_thread1 = SerialReaderThread(port='/dev/ttyACM1', baud=115200, idx=1)
