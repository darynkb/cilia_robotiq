...
bool Measure(byte address, int16_t *measurements) {
  uint8_t data[7];
  ...
  WireFlush();
  Serial.println("Measurement not read");
  return false;
}
...
void StaticCalibration() {
  ...
  for (int s = 0; s < totalSamples; s++) {
    for (int i = 0; i < 4; i++) {
      ...
      while (retries < 5) {
        bool success = Measure(sensor_base | sensors_array[i], measurement);
        if (success) { //(measurement[0] != 0 || measurement[1] != 0 || measurement[2] != 0) {
          valid = true;
          break;
        }
        retries++;
